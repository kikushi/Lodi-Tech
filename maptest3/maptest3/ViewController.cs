...
namespace maptest3
{
	public partial class ViewController : UIViewController
	{
		...
		public override void ViewDidLoad()
		{
			...
			map.AddAnnotation(new CustomAnnotation("Xamarin", mapCenter));

			// add an overlay
			var circleOverlay = MKCircle.Circle(mapCenter, 1000);
			map.AddOverlay(circleOverlay);

			var searchResultsControllerp = new SearchResultController(map);


			var searchUpdater = new SearchResultsUpdator();
			searchUpdater.UpdateSearchResults += searchResultsControllerp.Search;

			//add the search controller
			searchController = new UISearchController(searchResultsControllerp)
			{
